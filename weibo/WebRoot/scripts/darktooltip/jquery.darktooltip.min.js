(function(b) {
	...
	a.prototype = {
		...
		setContent : function() {
 			if (this.options.content) {
				...
				if (this.bearer.attr("data-tooltip")) {
					...
					console.log("No content for tooltip: "
							+ this.bearer.selector);
					return
				}
			}
			...
			this.tooltip = b("<ins class = 'dark-tooltip " + this.options.theme
					+ " " + this.options.size + " " + this.options.gravity
					+ "'><div>" + this.content
					+ "</div><div class = 'tip'></div></ins>");
			this.tip = this.tooltip.find(".tip");
			b(this.bearer).append(this.tooltip);
		...
		setPositions : function() {
			var d = 0;
			var c = 0;
			var f = this.bearer.offset().top;
			var e = this.bearer.offset().left;
			if (this.bearer.css("position") == "fixed"
					|| this.bearer.css("position") == "absolute") {
				f = 0;
				e = 0
			}
			switch (this.options.gravity) {
			case "south":
				d = e + this.bearer.outerWidth() / 2
						- this.tooltip.outerWidth() / 2;
				c = f - this.tooltip.outerHeight() - this.tip.outerHeight() / 2;
				break;
			case "west":
				d = e + this.bearer.outerWidth() + this.tip.outerWidth() / 2;
				c = f + this.bearer.outerHeight() / 2
						- (this.tooltip.outerHeight() / 2);
				break;
			case "north":
				d = e + this.bearer.outerWidth() / 2
						- (this.tooltip.outerWidth() / 2);
				c = f + this.bearer.outerHeight() + this.tip.outerHeight() / 2;
				break;
			case "east":
				d = e - this.tooltip.outerWidth() - this.tip.outerWidth() / 2;
				c = f + this.bearer.outerHeight() / 2
						- this.tooltip.outerHeight() / 2;
				break
			}
		...
		addConfirm : function() {
			this.tooltip
					.append("<ul class = 'confirm'><li class = 'darktooltip-yes'>"
							+ this.options.yes
							+ "</li><li class = 'darktooltip-no'>"
							+ this.options.no + "</li></ul>");
